...
describe('deepMerge', () => {
  ...
  describe('null, undefined, and empty string handling', () => {
    ...
    it('should skip empty strings from source when target has non-empty value', () => {
    ...
    it('should overwrite empty string in target with non-empty value from source', () => {
      const target = { a: 'hello', b: '' };
      const source = { b: 'world', c: 'test' };
      const result = deepMerge(target, source);
...
describe('deepMergeAll', () => {
  ...
  describe('token merge use case', () => {
    ...
    it('should prefer non-empty values from API over empty strings from DB', () => {
      const apiToken = {
        address: '0x123',
      ...
      const dbToken = {
        address: '0x123',
        pairAddress: '', // Empty string from DB
        name: 'DB Token',
        symbol: 'DBT', // Extra field only in DB
      };

      // Order: API first, DB last (like [gmgn, birdeye, dbToken])
      const result = deepMergeAll([apiToken, dbToken]);

      expect(result).toEqual({
        address: '0x123',
        pairAddress: '0xcompletedpair', // Should prefer API value over empty string
        name: 'API Token', // First non-empty wins
        symbol: 'DBT', // Only in DB, so it's included
      });
    });
  });
});
